...
const MechArray = [];
// read cookie for persistence
//var persistentMechs = document.cookie.indexOf('mechs=');
//if (persistentMechs != -1) {
//  alert(document.cookie);
//  importArmy("FromCookie");
//}
...
var searchMasterUnitList = '<div id="botmaindisplay" class="mainbox">\
<u>TDZ Table Top Bot v3.0</u><p></p><div style="margin:auto;display: block;">Enter name or variant of Mech</div><p></p><table class="center"><tr><td>\
<input type="text" id="mechSearchTerm" style="margin:auto;display: block;">\
</td></tr><tr><td>\
<button onclick="loadMasterUnitListContent()" class="button"  style="margin:auto;display: block;">Lookup Mech</button><br>\
<div id="selectedMechDisplay"></div><br>\
<button onclick="startScreenArmyManual()" class="button">Enter Details Manually</button><button onclick="loadArmyList()" class="button">Finish</button><p></p>\
</td></tr></tavble></div>';

//TBD: format Mech Image
var createArmyDisplay = '<div id="botmaindisplay" class="mainbox">\
  <u>TDZ Table Top Bot v3.0</u><p></p>\
    Add a Mech to your Army:<br>\
    <div id="MechCounter" class="smalltext">Mechs in Army: 0</div>\
    <img id="imageBox" src="images/bot2.png" class="mechimage">\
    <table class="center"><tr><td>\
    Name: </td><td colspan=3><input type="text" id="MechName"></td>\
    </tr><tr>\
    <td>MV: </td><td><input id="MechMV" type="text" class="miniinput2"></td>\
    <td>Skill: </td><td><input id="MechSkill" type="number" class="miniinput2"></td>\
    </tr></table>\
    ...
    <td><input id="MechS" type="number" class="miniinput2"></td><td><input id="MechM" type="number" class="miniinput2"></td><td><input id="MechL" type="number" class="miniinput2"></td>\
    </tr></table>\
    <div title="Enter the values for armor and inner structure." class="smalltext" style="margin-top:10px;">Protection:</div>\
    <table class="center"><tr><td>\
    A</td><td>S</td>\
    </tr><tr>\
    <td><input id="MechA" type="number" class="miniinput2"></td><td><input id="MechI" type="number" class="miniinput2"></td>\
    </tr></table>\
    ...
    \
    <button onclick="loadArmyList()" class="button">Finish (Discard)</button>\
...
function startScreenArmy() {
  ...
  document.getElementById("outbox").innerHTML = searchMasterUnitList;
}
function startScreenArmyManual() {
  document.getElementById("outbox").innerHTML = createArmyDisplay;
}
...
// This function connects to masterunitlist.com and pulls the required JSON files
// example taken from https://code-boxx.com/get-html-content-javascript-fetch/
function loadMasterUnitListContent() {
  var mechSearchTerm = document.getElementById("mechSearchTerm").value;
  fetch("https://masterunitlist.azurewebsites.net/Unit/QuickList?Name="+mechSearchTerm)

  .then(units => {
    if (units.status != 200) { throw new Error("Error"); }
    return units.json();
  })
...
function unitsJSONSelector(){
  if (mObject.Units.length != 0) {
    if (mObject.Units.length > 1) {
      var mstring = "<div style='margin:auto;display: block;'>Select Variant</div><p></p><select id='variantselector' style='margin:auto;display: block;'>";
      for (let i = 0; i < mObject.Units.length; i++) {
        mstring += "<option value="+i+">"+mObject.Units[i].Name+"</option>";
      }
      mstring += "</select><br>";
      mstring += '<input type="button" value="Select Variant" onclick="selectMechFromArray(document.getElementById(\'variantselector\').value)" class="button" style="margin:auto;display: block;">';
      document.getElementById("selectedMechDisplay").innerHTML = mstring;
    }else{
      selectMechFromArray(0);
    }
  } else {
    document.getElementById("selectedMechDisplay").innerHTML = "Not a valid Mech, Please try again";
  }
}

function selectMechFromArray(arrayNumber) {
  arrayNumber = Number(arrayNumber);
  document.getElementById("outbox").innerHTML = createArmyDisplay;
  document.getElementById("MechCounter").innerHTML = 'Mechs in Army: '+ MechArray.length;
  document.getElementById("imageBox").src = mObject.Units[arrayNumber].ImageUrl;
  document.getElementById("MechName").value = mObject.Units[arrayNumber].Name;
  document.getElementById("MechMV").value = mObject.Units[arrayNumber].BFMove;
  document.getElementById("MechS").value = mObject.Units[arrayNumber].BFDamageShort;
  document.getElementById("MechM").value = mObject.Units[arrayNumber].BFDamageMedium;
  document.getElementById("MechL").value = mObject.Units[arrayNumber].BFDamageLong ;
  document.getElementById("MechA").value = mObject.Units[arrayNumber].BFArmor;
  document.getElementById("MechI").value = mObject.Units[arrayNumber].BFStructure;
}
...
function initArmyList() {
  // empty cookie
  //document.cookie = "mechs=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
  //var cookie = "mechs=";
  ...
  for (var i = 0; i < MechArray.length; i++) {
    var image = MechArray[i].image;
    if (MechArray[i].image == "") {
      image = "images/bot2.png";
    }

    // create cookie
    //cookie += MechArray[i].exportAll() + "\n";
    // load list
    armyList += '<table class="center"><tr><td><img src="'+image+'" class="mechimagelist"></td><td>\
  ...
  // document.cookie = cookie + "; expires=Thu, 01 Jan 3000 00:00:00 UTC; path=/;";
  //alert(document.cookie);
  return armyList;
}
...
function createMech(nextOption) {
  ...
  var image = document.getElementById("imageBox").src;

  //create obkect and append
  var MechObject = new Mech(name, mv, short, medium, long, armor, innerS, skill, aggression, image);
  if (name != "") {
...
function exportArmy() {
  ...
  <button onclick="importArmy(\'import\')" class="dbutton">Import Textarea</button>\
...
function importArmy(decider){
  ...
  for(var i = 0;i < lines.length;i++){
    if (lines[i] != "") {
      ...
      var MechObject = new Mech(parts[0], parts[1], parts[2], parts[3], parts[4], parts[5], parts[6], parts[7], parts[8], parts[9]);
      MechArray.push(MechObject);
    }
  }
  if (MechArray.length > 0) {
...
class Mech {
  //constructor
  constructor(name, mv, short, medium, long, armor, innerS, skill, aggression, image) {
    this.name = name;
    this.mv = mv;
    this.short = short;
    this.medium = medium;
  ...
  exportAll(){
    var exportString = this.name+","+this.mv+","+this.short+","+this.medium+","+this.long+","+this.armor+","+this.innerS+","+this.skill+","+this.aggression+","+this.image;
    return exportString;
  }
